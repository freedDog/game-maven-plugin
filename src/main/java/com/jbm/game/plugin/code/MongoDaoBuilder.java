...
public class MongoDaoBuilder extends AbstractMojo{
	...
	private String createMongoDao(File file) throws Exception {
		...
		String classObject=StringUtil.toLowerCaseFirstOne(className);	//类对象
		...
		sb.append("import com.jbm.game.engine.mongo.AbsMongoManager;\n");
		...
		sb.append("\t\tsuper(").append(file.getName().substring(0, file.getName().indexOf("."))).append(
				".class, mongoManager.getMongoClient(), mongoManager.getMorphia(),mongoManager.getMongoClientConfig().getDbName());\n");
		sb.append("\t}\n\n");
